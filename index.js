...
var RNImmediatePhoneCall = {
  immediatePhoneCall: async function (
    number,
    ...
    if (isAndroid) {
      if (checkPermissions) {
        try {
          ...
          if (!permission) {
            let granted = await requestCallPermission(
              title,
              message,
              buttonNegative,
              buttonPositive
            ...
            if (granted) {
              return NativeModules.RNImmediatePhoneCall.immediatePhoneCall(
                number
              );
            }
  ...
  canCall: async function () {
    return NativeModules.RNImmediatePhoneCall.canCall();
  },
};
